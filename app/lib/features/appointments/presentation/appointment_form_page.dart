...
class _AppointmentFormPageState extends ConsumerState<AppointmentFormPage> {
  ...
  final List<String> _serviceTypes = ['Tattoo', 'Piercing', 'Consultation', 'Touch-up'];
  final List<String> _statuses = ['Scheduled', 'In Progress', 'Completed', 'Cancelled', 'No Show'];
  ...
  Future<void> _save() async {
    ...
    if (_selectedClient == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('You must summon a soul (select client).')),
      );
      return;
    }
    ...
    try {
       ...
       final userId = authState.value?.mapOrNull(
       ...
       if (widget.appointmentId != null) { // Update
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(widget.appointmentId == null ? 'New Ritual' : 'Edit Ritual'),
        backgroundColor: InfernalColors.surface,
           ...
           children: [
             ...
             InkWell(
               ...
               child: Container(
                 ...
                 decoration: BoxDecoration(
                   ...
                   borderRadius: BorderRadius.circular(InfernalRadius.md),
                   border: Border.all(
                      color: _selectedClient == null ? InfernalColors.border : InfernalColors.arcane
                   ),
                 ),
                 child: Row(
                   children: [
                     ...
                     Expanded(
                        child: Text(
                          _selectedClient == null ? 'Select Soul (Client)' : _selectedClient!.fullName,
                          style: TextStyle(
                             color: _selectedClient == null ? InfernalColors.textMuted : InfernalColors.textPrimary,
                             fontSize: 16,
                          ),
                        ),
                     ),
                     const Icon(Icons.arrow_drop_down, color: InfernalColors.textMuted),
                   ],
                 ),
               ),
             ),
                   ...
                   child: InkWell(
                     ...
                     child: InputDecorator(
                       decoration: const InputDecoration(
                         ...
                         prefixIcon: Icon(Icons.calendar_today, color: InfernalColors.blood),
                       ),
                       child: Text(
                   ...
                   child: InkWell(
                     ...
                     child: InputDecorator(
                       decoration: const InputDecoration(
                         ...
                         filled: true,
                         fillColor: InfernalColors.surface,
                         prefixIcon: Icon(Icons.access_time, color: InfernalColors.blood),
                       ),
                       child: Text(
               ...
               children: [
                 Expanded(
                   child: DropdownButtonFormField<String>(
                      initialValue: _serviceTypes.contains(_serviceType) ? _serviceType : _serviceTypes.first,
             ...
             TextFormField(
               ...
               decoration: const InputDecoration(
                 ...
                 fillColor: InfernalColors.surface,
                 border: OutlineInputBorder(),
                 prefixIcon: Icon(Icons.timer, color: InfernalColors.textSecondary),
               ),
               validator: (val) => (int.tryParse(val ?? '') ?? 0) > 0 ? null : 'Invalid duration',
             ),
             const SizedBox(height: InfernalSpacing.lg),

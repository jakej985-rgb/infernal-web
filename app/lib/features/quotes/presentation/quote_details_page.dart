        ...
        data: (quote) {
           ...
           return ListView(
              ...
              children: [
                          ...
                          Row(
                             ...
                             children: [
                                Expanded(
                                  child: Text(
                                     quote.placement, 
                                     style: Theme.of(context).textTheme.headlineSmall?.copyWith(color: InfernalColors.textPrimary, fontWeight: FontWeight.bold)
                                  ),
                                ),
                                if (quote.isCoverUp)
                                  Container(
                                    padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                                    decoration: BoxDecoration(color: InfernalColors.voidColor, borderRadius: BorderRadius.circular(4)),
                                    child: const Text('COVER-UP', style: TextStyle(color: InfernalColors.blood, fontSize: 12, fontWeight: FontWeight.bold)),
                                  ),
                             ],
                          ),
                          const SizedBox(height: 4),
                          Text(quote.style, style: const TextStyle(color: InfernalColors.textSecondary, fontSize: 18)),
                          const SizedBox(height: InfernalSpacing.md),
                          Row(
                             children: [
                                _Badge(icon: Icons.aspect_ratio, label: '${quote.width}" x ${quote.height}"'),
                                const SizedBox(width: InfernalSpacing.md),
                                _Badge(icon: Icons.calendar_today, label: DateFormat('MM/dd/yy').format(quote.createdAt)),
                             ],
                          ),
                       ],
                    ),
                 ...
                 Container(
                    ...
                    decoration: BoxDecoration(
                       gradient: const LinearGradient(
                          colors: [InfernalColors.surface, InfernalColors.surfaceElevated],
                          begin: Alignment.topLeft,
                          end: Alignment.bottomRight,
                       ),
                       borderRadius: BorderRadius.circular(InfernalRadius.md),
                       border: Border.all(color: InfernalColors.gold.withValues(alpha: 0.3)),
                       boxShadow: [
                          BoxShadow(color: InfernalColors.gold.withValues(alpha: 0.05), blurRadius: 10, spreadRadius: 0),
                       ],
                    ),
                    child: Column(
                       crossAxisAlignment: CrossAxisAlignment.center,
                       children: [
                          const Text('ESTIMATED PRICE', style: TextStyle(color: InfernalColors.gold, letterSpacing: 1.5, fontSize: 12, fontWeight: FontWeight.bold)),
                          const SizedBox(height: InfernalSpacing.sm),
                          Text(
                             '\$${quote.priceLow.toStringAsFixed(0)} - \$${quote.priceHigh.toStringAsFixed(0)}',
                             style: const TextStyle(color: InfernalColors.textPrimary, fontSize: 32, fontWeight: FontWeight.bold),
                          ),
                          ...
                          const SizedBox(height: InfernalSpacing.md),
                          Text('Recommended Deposit: \$${quote.recommendedDeposit.toStringAsFixed(0)}', style: const TextStyle(color: InfernalColors.textMuted)),
                       ],
                    ),
                 ),
                 const SizedBox(height: InfernalSpacing.lg),
                 ...
                 // Factors Grid
                 const Text("Complexity Factors", style: TextStyle(color: InfernalColors.textSecondary, fontWeight: FontWeight.bold)),
                 const SizedBox(height: InfernalSpacing.sm),
                 GridView.count(
                    crossAxisCount: 2,
                    shrinkWrap: true,
                 ...
                 if (quote.notes != null && quote.notes!.isNotEmpty) ...[
                    const SizedBox(height: InfernalSpacing.lg),
                    const Text("Notes", style: TextStyle(color: InfernalColors.textSecondary, fontWeight: FontWeight.bold)),
                    const SizedBox(height: InfernalSpacing.sm),
                    Container(
                      padding: const EdgeInsets.all(InfernalSpacing.md),
                      color: InfernalColors.surface,
                      width: double.infinity,
                      child: Text(quote.notes!, style: const TextStyle(color: InfernalColors.textPrimary)),
                    ),
                 ]
              ],
           );
...
class _Factor extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
       children: [
          Expanded(child: Text(label, style: const TextStyle(color: InfernalColors.textMuted))),
          Container(
             width: 30, height: 30,
             ...
             decoration: BoxDecoration(
                ...
                border: Border.all(color: value > 3 ? InfernalColors.blood : InfernalColors.border),
             ),
             child: Text(value.toString(), style: TextStyle(color: value > 3 ? InfernalColors.blood : InfernalColors.textPrimary, fontWeight: FontWeight.bold)),
          ),
       ],
    );
  }
